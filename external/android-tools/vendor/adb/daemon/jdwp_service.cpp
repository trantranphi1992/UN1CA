...
static bool is_process_in_freezer(const ProcessInfo& info) {
    ...
    if (!CgroupGetAttributePathForProcess("FreezerState", info.uid, info.pid, path)) {
        VLOG(JDWP) << fmt::format("Failed to build frozen path of '{}' (got '{}')", info.pid, path);
        return false;
    }

    std::string content;
    if (!android::base::ReadFileToString(path, &content)) {
        VLOG(JDWP) << fmt::format("Failed to read ({})", path);
        return false;
    }

    bool is_frozen = content == "1\n";
    VLOG(JDWP) << fmt::format("Checking if pid {} is frozen at '{}' = '{}'", info.pid, path,
                              content);

    if (is_frozen) {
        LOG(WARNING) << fmt::format("According to '{}'(='{}'), pid {} is frozen", path, content,
                                    info.pid);
    }
    return is_frozen;
}
...
static unique_fd send_socket_to_process(JdwpProcess& proc) {
    ...
    if (is_process_in_freezer(proc.process)) {
        ...
        LOG(WARNING) << fmt::format("Process {} ({}) is frozen. Denying JDWP connection", info.pid,
                                    info.process_name);
        return unique_fd{};
    }
    ...
    if (adb_socketpair(fds) < 0) {
        LOG(WARNING) << fmt::format("{}: socket pair creation failed: {}", __FUNCTION__,
                                    strerror(errno));
        return unique_fd{};
    }

    VLOG(JDWP) << fmt::format("socketpair: ({},{})", fds[0], fds[1]);
...
unique_fd create_jdwp_connection_fd(pid_t pid) {
    VLOG(JDWP) << fmt::format("looking for pid {} in JDWP process list", pid);
    ...
    LOG(WARNING) << fmt::format("search for pid {} failed !!", pid);
    return unique_fd{};
}
